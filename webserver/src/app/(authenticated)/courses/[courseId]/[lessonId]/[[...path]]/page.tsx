...
import CourseMaterialTable, {
  BucketFile,
} from "@/components/ui/CourseMaterialTable";
...
export default async function Page({
  ...
  if (files.length + prefixes.length > 1) {
    ...
    const prefixFiles = prefixes.map(
      (p) =>
        ({
          name: p
            .split("/")
    ...
    return (
      <Component
        markdown={markdown}
        childrenFiles={[...prefixFiles, ...bucketFiles]}
        path={fullPath}
      />
    );
  }
