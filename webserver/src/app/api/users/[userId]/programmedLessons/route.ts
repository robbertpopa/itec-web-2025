  ...
  try {
    ...
    if (requestUserId !== userId) {
      return NextResponse.json(
        {
          error:
            "Unauthorized: Cannot update another user's programmed lessons",
        },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: isMarked
          ? "Lesson day marked successfully"
          : "Lesson day unmarked successfully",
        date: dateISOString,
      },
      { status: 200 }
    );
  ...
  try {
    ...
    if (requestUserId !== userId) {
      return NextResponse.json(
        {
          error:
            "Unauthorized: Cannot access another user's programmed lessons",
        },
        { status: 403 }
      );
    }
